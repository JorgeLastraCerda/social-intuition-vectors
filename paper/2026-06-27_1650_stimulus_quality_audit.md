...
The 200 concept stories are **high-quality stimuli** by the standards of behavioral and mechanistic-interpretability research. Structural balance is excellent across all measurable dimensions; the minimal-pair design is textbook clean; name and demographic leakage is absent; axis-label leakage is negligible (2/200 stories). Narrative quality (show-don't-tell, behavioral anchoring, specificity) is strong for a machine-generated corpus.

**Overall score: 8.5 / 10.**

The two genuine weaknesses — mono-source generation (all stories from a single model, `claude-opus-4-8`) and absence of independent human validity checks — do not invalidate any existing result but represent the clearest opportunities for strengthening the corpus before journal submission.
...
### What needs a caveat (or future-work item)

1. **Mono-source generation (most important).** One sentence in the Stimuli section of the paper: *"All stories were generated by a single model (Claude Opus 4.8); a cross-model holdout study would strengthen claims about generalizability beyond this generator's stylistic conventions."* This is standard practice for machine-generated stimuli — it does not require pre-submission work but must be disclosed.

2. **Human validity study (recommended before final submission).** A rating study in which 20–30 human annotators rate a random 40-story sample (10 per condition) on 7-point warmth and competence scales would: (a) confirm that the intended manipulation is perceived as intended; (b) provide a manipulation check table for the paper. Cost: 1–2 weeks of Prolific / MTurk data collection. This is optional for a first-submission draft but expected at revision.

3. **Warmth/competence axis correlation.** The ~41–59° inter-axis angle (not 90°) reflects the known SCM correlation. This should be reported as an empirical finding, not a corpus flaw: *"Warmth and competence are not orthogonal in the model's representational geometry — consistent with the Stereotype Content Model's prediction of a slight negative real-world correlation."*

4. **Low condition verbosity.** A one-sentence footnote noting that low-condition stories are ~0.7–1.2 sentences longer on average, and that the probe operates on position-level activations rather than length-pooled representations, is sufficient to preempt this concern.

---

## 6. Recommended next steps (priority order)

1. **(Before submission — low cost)** Add a 40-story human rating sub-study for manipulation check. Prolific run, ~£80, ~2 weeks.
2. **(Before submission — medium cost)** Generate a 20-story holdout set using a different generator (e.g., Gemma-27B, GPT-4o) and run the probe on it. Tests whether the probe generalizes across generator styles. Script already supports new story inputs via `--stories-path`.
3. **(Optional enrichment)** Expand from 50 to 80 topics for a larger, more representative corpus. The generation prompt is in `STIMULI_TRACKER.md`; incremental cost is negligible.
