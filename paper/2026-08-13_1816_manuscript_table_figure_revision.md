...
- **Scripts:** `src/build_paper_probe_tables.py`, `src/build_paper_mediation_table.py`, `paper/figures/_steering_transition_flow_common.py`, `paper/figures/fig6_cross_model_agreement.py`, `paper/figures/generate_figures.py`, `paper/figures/background_emotion_vector.py`, `paper/figures/background_concept_geometry.py`, `paper/figures/paper_figure1_axis_arrows.py`, `paper/figures/paper_figure2_layer_emergence.py`, `paper/figures/paper_figure4_hiring_bidirectional_examples.py`
- **Inputs:** `results/logs/hiring_probe_vs_human_*.json`, `results/logs/validate_probes_*.json`, `results/logs/split_half_stability_*.json`, `results/logs/hiring_mediation_*.json`, `results/tables/hiring_audit_*.csv`, `results/tables/hiring_steering_*.csv`, `results/tables/hiring_steering_calibrated_*.csv`, `results/tables/hiring_disparity_*.csv`, `results/tables/cross_model_agreement_9model.csv`, `results/tables/gemma_scope_*.csv`, `published_data/df_all.csv`
...
- Seven models favor Black-signaling names and two favor White-signaling names after within-model standardization. Eight models favor female-signaling names, opposite to the pooled human gender reference. The pooled human race reference is near zero at $+1.2$ percentage points.
...
Model callback gaps are reported in within-model standard-deviation units, while the human benchmark remains in percentage points. The mediation table stays as a full-page Supplementary Materials table. Two related-work records were integrated into the argument rather than listed without context. All active figures were regenerated through the repository scripts and shared serif styling. The final 35-page PDF has no overfull boxes, undefined references, unresolved citations, or float-too-large warnings.
