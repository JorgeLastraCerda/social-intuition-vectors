...
is presented through each model's native chat template; the other four
models receive it as raw text, matching how each checkpoint was run
throughout.
...
roster. Strength is swept broadly at $\alpha \in \{\pm 0.25,\,\pm 0.50\}$ on
all nine models and refined at $\{\pm 0.05,\,\pm 0.10\}$ on every checkpoint
except Llama-3.1-8B and Qwen3-14B, so that a push on the warmth or
competence direction can be traced directly to a shift in the callback
margin.
