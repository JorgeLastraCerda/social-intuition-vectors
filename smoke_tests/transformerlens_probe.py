"""Smoke test — TransformerLens, any natively supported model.

Runs the same 50+50 warm/cold probe pipeline used for the Gemma 3 12B baseline.
The probe logic, assertions, and output format are identical across model families;
only the --model, --label, and --out-dir differ between runs.

Run from repo root:
    python smoke_tests/transformerlens_probe.py \\
        --model Qwen/Qwen3-14B --label qwen3_14b \\
...
Pass criterion: probe_cv_mean > 0.80  (same as the Gemma 12B smoke).
...
# -- path setup --------------------------------------------------------------
HERE  = Path(__file__).resolve().parent   # smoke_tests/
...
# ---------------------------------------------------------------------------
# Helpers  (identical to gemma3_transformerlens version — same TL API)
# ---------------------------------------------------------------------------
...
def run(config_path: str, model_name: str, label: str, start_token: int,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Smoke test — TransformerLens, 50+50 warm/cold probe, any supported model."
    )
    parser.add_argument("--config", default="config/config.yaml")
    parser.add_argument("--model",  required=True,
                        help="HF model id (must be in TransformerLens OFFICIAL_MODEL_NAMES, "
                             "e.g. Qwen/Qwen3-14B, meta-llama/Llama-3.1-8B-Instruct).")
    parser.add_argument("--label",  required=True,
                        help='Short tag written to the JSON \"test\" field (e.g. qwen3_14b).')
    parser.add_argument("--out-dir", required=True,
                        help="Directory for saved vectors and JSON log "
                             "(e.g. smoke_tests/results/qwen3_14b).")
    ...
    try:
        result = run(args.config, args.model, args.label,
                     args.start_token, args.seed, out_dir)
