#!/bin/bash
# SGE job: extract neutral-corpus activations for Llama-3.1-8B-Instruct, then
# PCA-denoise its warmth/competence vectors.
#
...
# same model/vectors-subdir already used in jobs/sge/extract_llama31_8b.sh.
#
# Closes a coverage gap found 2026-08-04: Llama-3.1-8B-Instruct and Qwen3-14B
...
#
# Model: meta-llama/Llama-3.1-8B-Instruct (~16 GB bf16) — fits comfortably on
# an L40 (48 GB). Weights already cached under HF_HOME on SCCKN; no download.
# Requires data/processed/concept_vectors_llama31_8b/{warmth_vec,competence_vec,
...
#$ -N wc_extract_neutral_llama31_8b
#$ -q gpu@scc213,gpu@scc214
#$ -l h_rt=01:00:00
#$ -l h_vmem=64G
#$ -pe smp 2
#$ -l gpu=1
#$ -o results/logs/extract_neutral_llama31_8b.out
#$ -e results/logs/extract_neutral_llama31_8b.err
#$ -cwd
#$ -m ea
...
mkdir -p results/logs data/processed/concept_vectors_llama31_8b

echo "[job] Step 1: extract neutral-corpus activations — Llama-3.1-8B-Instruct (GPU)"
