#!/bin/bash
# SGE job: Phase 7 hiring audit — Llama-3.1-8B-Instruct.
#
# Submit only after the Gemma-3-12B regression gate passes.
# Model: meta-llama/Llama-3.1-8B-Instruct (~16 GB bf16) — fits L40 48 GB nodes.
#
# Outputs:
...
#   results/logs/hiring_steering_llama31_8b.json
...
#$ -pe smp 2                  # ADJUST
#$ -l gpu=1
#$ -o results/logs/hiring_llama31_8b.out
#$ -e results/logs/hiring_llama31_8b.err
#$ -cwd
#$ -m ea
...
echo "[job] Phase 7 hiring steering — Llama-3.1-8B-Instruct"
...
echo "[job] Phase 7 hiring audit — Llama-3.1-8B-Instruct"
python -m src.hiring_audit \
    --config config/config.yaml \
    --vectors-subdir concept_vectors_llama31_8b \
    --label llama31_8b

echo "[job] Phase 7 hiring disparity + mediation — Llama-3.1-8B-Instruct (CPU)"
...
echo "[job] Llama-3.1-8B hiring audit complete."
